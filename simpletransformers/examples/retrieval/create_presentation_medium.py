"""
Create Presentable Results for Medium Dataset Project
...
def create_presentation_results():
    """Create a presentable results document for medium dataset."""
    
    results_file = os.path.join(script_dir, "baseline_medium_evaluation_results.json")
    
    if not os.path.exists(results_file):
        print(f"ERROR: Results file not found at {results_file}")
        print("Please run evaluate_baseline_medium.py first.")
        return
    ...
    output_file = os.path.join(script_dir, "BASELINE_MEDIUM_RESULTS_PRESENTATION.md")
    
    with open(output_file, 'w') as f:
        f.write("# Baseline DPR Model Evaluation Results (Medium Dataset)\n\n")
        f.write("## Executive Summary\n\n")
        f.write("This document presents the evaluation results for the Baseline Dense Passage Retrieval (DPR) model ")
        f.write("trained on a medium-sized subset of the mMARCO (Multilingual MS MARCO) dataset. ")
        f.write("The model was trained on **5,000 examples** and evaluated on **500 validation examples**.\n\n")
        ...
        f.write("|-----------|-------|\n")
        f.write("| **Model Type** | Dense Passage Retrieval (DPR) |\n")
        f.write("| **Training Dataset** | mMARCO (Multilingual MS MARCO) |\n")
        f.write("| **Training Examples** | 5,000 |\n")
        f.write("| **Evaluation Dataset** | mMARCO Validation Set |\n")
        f.write("| **Evaluation Examples** | 500 |\n")
        ...
        f.write("1. **BASELINE_MEDIUM_RESULTS_PRESENTATION.md** (this file) - Markdown presentation\n")
        f.write("2. **baseline_medium_results_latex.tex** - LaTeX table for paper inclusion\n")
        f.write("3. **baseline_medium_results_table.csv** - CSV format for Excel/Google Sheets\n")
        f.write("4. **baseline_medium_evaluation_results.json** - Complete results in JSON format\n")
        f.write("\n---\n\n")
        f.write("*Generated from baseline DPR model evaluation on mMARCO medium validation set*\n")
    
    print(f"Presentation results saved to: {output_file}")
    
    # Also create a LaTeX table format
    latex_file = os.path.join(script_dir, "baseline_medium_results_latex.tex")
    with open(latex_file, 'w') as f:
        f.write("\\begin{table}[h]\n")
        f.write("\\centering\n")
        f.write("\\caption{Baseline DPR Model Evaluation Results (Medium Dataset)}\n")
        f.write("\\label{tab:baseline_medium_results}\n")
    ...
    csv_file = os.path.join(script_dir, "baseline_medium_results_table.csv")
    data = {
        'k': [],
        'MRR@k': [],
        'Recall@k': [],
