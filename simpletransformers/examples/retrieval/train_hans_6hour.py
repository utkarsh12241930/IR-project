"""
HANS DPR training on 6-hour dataset subset
"""
import os
import sys
...
# Specifying the path to the training data (6-hour subset)
train_data_path = os.path.join(script_dir, "data/mmarco/mmarco-train-6hour.tsv")
...
model_args.num_train_epochs = 1  # Single epoch for 6-hour window
model_args.train_batch_size = 16
model_args.eval_batch_size = 16
model_args.use_hf_datasets = True
model_args.learning_rate = 2e-6
model_args.warmup_steps = 50  # Reduced warmup for smaller dataset
...
model_args.output_dir = "trained_models/hans_dpr_6hour"
...
if __name__ == "__main__":
    print("=" * 50)
    print("TRAINING HANS DPR (6-HOUR DATASET)")
    print("=" * 50)
    print(f"Training examples: {len(train_data)}")
    print(f"Validation examples: {len(eval_data)}")
    print(f"HANS: ENABLED")
    print(f"Estimated training time: ~5-6 hours (CPU)")
    print("=" * 50)
