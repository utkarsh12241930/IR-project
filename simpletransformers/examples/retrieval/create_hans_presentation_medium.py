"""
Create Presentable Results for HANS Model (Medium Dataset)
...
def create_presentation_results():
    """Create a presentable results document for HANS model on medium dataset."""
    
    results_file = os.path.join(script_dir, "hans_medium_evaluation_results.json")
    
    if not os.path.exists(results_file):
        print(f"ERROR: Results file not found at {results_file}")
        print("Please run evaluate_hans_medium.py first.")
        return
    ...
    output_file = os.path.join(script_dir, "HANS_MEDIUM_RESULTS_PRESENTATION.md")
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# HANS DPR Model Evaluation Results (Medium Dataset)\n\n")
        f.write("## Executive Summary\n\n")
        f.write("This document presents the evaluation results for the HANS (Hardness-Adaptive Negative Sampling) ")
        f.write("Dense Passage Retrieval (DPR) model trained on a medium-sized subset of the mMARCO (Multilingual MS MARCO) dataset. ")
        ...
        f.write("1. **HANS_MEDIUM_RESULTS_PRESENTATION.md** (this file) - Markdown presentation\n")
        f.write("2. **hans_medium_results_latex.tex** - LaTeX table for paper inclusion\n")
        f.write("3. **hans_medium_results_table.csv** - CSV format for Excel/Google Sheets\n")
        f.write("4. **hans_medium_evaluation_results.json** - Complete results in JSON format\n")
        f.write("\n---\n\n")
        f.write("*Generated from HANS DPR model evaluation on mMARCO medium validation set*\n")
    
    print(f"Presentation results saved to: {output_file}")
    
    # Also create a LaTeX table format
    latex_file = os.path.join(script_dir, "hans_medium_results_latex.tex")
    with open(latex_file, 'w', encoding='utf-8') as f:
        f.write("\\begin{table}[h]\n")
        f.write("\\centering\n")
        f.write("\\caption{HANS DPR Model Evaluation Results (Medium Dataset)}\n")
        f.write("\\label{tab:hans_medium_results}\n")
    ...
    csv_file = os.path.join(script_dir, "hans_medium_results_table.csv")
    data = {
        'k': [],
        'MRR@k': [],
        'Recall@k': [],
