"""
Evaluate HANS DPR Model (Medium Dataset) with F2 Score
This script evaluates the HANS model trained on medium dataset.
"""
import os
import sys
import json
...
def evaluate_hans_medium(model_path, eval_data_path, top_k_values=[1, 5, 10, 20, 50, 100]):
    """Evaluate HANS model with F2 score and other metrics."""
    print("="*80)
    print("HANS DPR MODEL EVALUATION (Medium Dataset)")
    print("="*80)
    ...
    print(f"\nEvaluation complete!")
    print("="*80)
    print("EVALUATION RESULTS - HANS DPR (MEDIUM DATASET)")
    print("="*80)
    ...
    # Save results
    output_file = os.path.join(script_dir, "hans_medium_evaluation_results.json")
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to: {output_file}")
    
    # Save summary in research paper format
    summary_file = os.path.join(script_dir, "hans_medium_evaluation_summary.txt")
...
if __name__ == "__main__":
    model_path = os.path.join(script_dir, "trained_models", "hans_dpr_medium")
    ...
    evaluate_hans_medium(model_path, eval_data_path)
