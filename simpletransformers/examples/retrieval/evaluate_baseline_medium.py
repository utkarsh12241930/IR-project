"""
Evaluate Baseline DPR Model (Medium Dataset) with F2 Score
This script evaluates the baseline model trained on medium dataset.
"""
import os
import sys
import json
...
def evaluate_baseline_medium(model_path, eval_data_path, top_k_values=[1, 5, 10, 20, 50, 100]):
    """Evaluate baseline model with F2 score and other metrics."""
    print("="*80)
    print("BASELINE DPR MODEL EVALUATION (Medium Dataset)")
    print("="*80)
    ...
    print(f"\nEvaluation complete!")
    print("="*80)
    print("EVALUATION RESULTS - BASELINE DPR (MEDIUM DATASET)")
    print("="*80)
    ...
    # Save results
    output_file = os.path.join(script_dir, "baseline_medium_evaluation_results.json")
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to: {output_file}")
    
    # Save summary in research paper format
    summary_file = os.path.join(script_dir, "baseline_medium_evaluation_summary.txt")
...
if __name__ == "__main__":
    model_path = os.path.join(script_dir, "trained_models", "baseline_dpr_medium")
    ...
    evaluate_baseline_medium(model_path, eval_data_path)
