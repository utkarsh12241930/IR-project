...
### Hardness-Adaptive Negative Sampling (HANS)

When working on multilingual dense passage retrieval over MS MARCO-style datasets, we introduce **Hardness-Adaptive Negative Sampling (HANS)** as an optional add-on inside `RetrievalModel`. HANS focuses purely on *novelty*—highlighting that we treat linguistic difficulty as a first-class signal without changing the rest of the DPR workflow.

- **Hardness predictor:** a lightweight heuristic + MLP module that estimates how ambiguous or rare each query is (e.g., code-mixing, morphology or rarity cues).
- **Difficulty-aware negatives:** the harder the query, the more aggressive the negative mining. Easy queries keep a softer curriculum; difficult ones enable deeper hard-negative pools or drop uninformative negatives entirely.
- **Adaptive contrastive weighting:** hardness also scales the contrastive loss (and optional margin / temperature) so ambiguous queries receive stronger supervision while simple queries avoid overfitting.
- **Dynamic curriculum:** the hardness score feeds into a scheduled curriculum, letting multilingual DPR focus first on easy queries, then gradually emphasize linguistically challenging ones.

This is, to our knowledge, the only multilingual DPR setting that dynamically adjusts *negative sampling difficulty, contrastive weighting, and retrieval margins* based on continuous hardness estimates. The rest of your pipeline (tokenization, batching, MS MARCO loading, etc.) remains unchanged; simply enable the new `use_hans` flag in `RetrievalArgs` to highlight this novelty in your experiments.
