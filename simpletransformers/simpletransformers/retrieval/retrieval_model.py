...
from simpletransformers.retrieval.hardness import HardnessPredictor
...
class RetrievalModel:
    def __init__(
        ...
        **kwargs,
        ...
        self.hans_enabled = self.args.use_hans
        self.hardness_predictor = None
        if self.hans_enabled:
            if self.args.hard_negatives:
                self.args.hans_max_negatives = min(
                    max(self.args.n_hard_negatives, self.args.hans_min_negatives),
                    self.args.n_hard_negatives,
                )
            else:
                self.args.hans_max_negatives = 0
            self._initialize_hans_predictor(model_name)
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if self.args.evaluate_during_training:
            if eval_data is None or (hasattr(eval_data, 'empty') and eval_data.empty):
                raise ValueError(
    ...
    def train(
        ...
        **kwargs,
        ...
        if self.hans_enabled and self.hardness_predictor is not None:
            self.hardness_predictor.zero_grad()
        if self.hans_enabled and self.hardness_predictor is not None:
            self.hardness_predictor.zero_grad()
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.optimizer == "AdamW":
                        ...
                        if self.hans_enabled and self.hardness_predictor is not None:
                            torch.nn.utils.clip_grad_norm_(
                                self.hardness_predictor.parameters(), args.max_grad_norm
                            )
                    ...
                    if (
                        ...
                        if args.wandb_project or self.is_sweeping:
                            ...
                            if retrieval_output.hardness_loss is not None:
                                logging_dict["hardness_loss"] = (
                                    retrieval_output.hardness_loss
                                )
    ...
    def get_optimizer_parameters(
        ...
        if self.hans_enabled and self.hardness_predictor is not None:
            optimizer_grouped_parameters.append(
                {"params": self.hardness_predictor.parameters(), "weight_decay": args.weight_decay}
            )
    ...
    def _calculate_loss(
        ...
        kl_div_loss = None
        quartet_loss = None
        hardness_targets = None
        hardness_prediction = None
        contrastive_weights = None
        hans_temperatures = None
        hans_margins = None

        if self.hans_enabled and self.hardness_predictor is not None:
            hardness_targets = teacher_scores.get("hardness_targets")
            if hardness_targets is not None:
                hardness_targets = hardness_targets.to(query_outputs.device).float()
            else:
                hardness_targets = torch.zeros(
                    query_outputs.size(0), device=query_outputs.device
                )
            contrastive_weights = self._get_hans_contrastive_weights(hardness_targets)
            hans_temperatures = self._get_hans_temperature(hardness_targets)
            hans_margins = self._get_hans_margin(hardness_targets)
            hardness_prediction = self.hardness_predictor(query_outputs)
        ...
        if self.args.include_triplet_loss:
            ...
            similarity_for_loss = similarity_score
            if self.hans_enabled and self.hardness_predictor is not None:
                similarity_for_loss = self._apply_hans_negative_mask(
                    similarity_for_loss, hardness_targets
                )
                similarity_for_loss = self._apply_hans_margin(
                    similarity_for_loss, labels, hans_margins
                )
                similarity_for_loss = self._apply_hans_temperature(
                    similarity_for_loss, hans_temperatures
                )
            softmax_score = torch.nn.functional.log_softmax(
                similarity_for_loss, dim=-1
            )
            ...
            if self.args.include_bce_loss and self.context_encoder.training:
                ...
                if self.args.mse_loss:
                    raise ValueError("Use of MSE loss is not supported.")
                    with torch.no_grad():
        ...
        if (
            self.hans_enabled
            and hardness_prediction is not None
            and hardness_targets is not None
        ):
            hardness_loss = torch.nn.functional.smooth_l1_loss(
                hardness_prediction, hardness_targets
            )
            loss = loss + self.args.hans_hardness_loss_weight * hardness_loss
    ...
    def _hans_scale(self, hardness, min_value, max_value, reverse=False):
        if hardness is None:
            return None
        if reverse:
            hardness = 1.0 - hardness
        return min_value + (max_value - min_value) * hardness

    def _get_hans_contrastive_weights(self, hardness):
        weights = self._hans_scale(
            hardness,
            float(self.args.hans_contrastive_weight_min),
            float(self.args.hans_contrastive_weight_max),
            reverse=True,
        )
        if weights is None:
            return None
        return torch.clamp(weights, min=1e-3)

    def _get_hans_temperature(self, hardness):
        temps = self._hans_scale(
            hardness,
            float(self.args.hans_temperature_min),
            float(self.args.hans_temperature_max),
            reverse=False,
        )
        if temps is None:
            return None
        return torch.clamp(temps, min=1e-3)

    def _get_hans_margin(self, hardness):
        margins = self._hans_scale(
            hardness,
            float(self.args.hans_margin_min),
            float(self.args.hans_margin_max),
            reverse=True,
        )
        if margins is None:
            return None
        return torch.clamp(margins, min=0.0)

    def _apply_hans_temperature(self, similarity, temperatures):
        if temperatures is None:
            return similarity
        temps = temperatures.unsqueeze(1)
        return similarity / torch.clamp(temps, min=1e-6)

    def _apply_hans_margin(self, similarity, labels, margins):
        if margins is None:
            return similarity
        one_hot = torch.nn.functional.one_hot(
            labels, num_classes=similarity.size(1)
        ).float()
        return similarity + one_hot * margins.unsqueeze(1)

    def _apply_hans_negative_mask(self, similarity, hardness_targets):
        if (
            not self.args.hard_negatives
            or self.args.hans_max_negatives == 0
            or hardness_targets is None
        ):
            return similarity
        batch_size = similarity.size(0)
        expected_cols = batch_size * (1 + self.args.n_hard_negatives)
        if similarity.size(1) != expected_cols:
            return similarity
        active_counts = self._get_hans_active_negatives(hardness_targets)
        updated = similarity.clone()
        for neg_idx in range(self.args.n_hard_negatives):
            block_start = batch_size * (neg_idx + 1)
            block_end = block_start + batch_size
            mask = active_counts <= neg_idx
            if mask.any():
                updated[mask, block_start:block_end] -= self.args.hans_neg_dropout_value
        return updated

    def _get_hans_active_negatives(self, hardness_targets):
        min_neg = max(1, self.args.hans_min_negatives)
        max_neg = max(min_neg, self.args.hans_max_negatives)
        counts = self._hans_scale(
            hardness_targets.float(), float(min_neg), float(max_neg), reverse=True
        )
        counts = torch.clamp(torch.round(counts), min=min_neg, max=max_neg)
        return counts.long()
    ...
    def _get_loss(
        ...
        if self.args.include_nll_loss:
            if contrastive_weights is not None:
                nll_per_example = torch.nn.functional.nll_loss(
                    softmax_score, labels, reduction="none"
                )
                weights = contrastive_weights.to(nll_per_example.device)
                denom = torch.clamp(weights.sum(), min=1e-8)
                nll_loss = (nll_per_example * weights).sum() / denom
            else:
                criterion = torch.nn.NLLLoss(reduction="mean")
    ...
    def _get_inputs_dict(self, batch, evaluate=False):
        ...
        if hardness_scores is None and "hardness_score" in batch:
            hardness_scores = batch["hardness_score"].to(device).float()
        if hardness_scores is not None:
            teacher_scores["hardness_targets"] = hardness_scores
    ...
    def save_model(
        ...
        if context_model and query_model and not self.args.no_save:
            ...
            if self.hans_enabled and self.hardness_predictor is not None:
                os.makedirs(
                    os.path.join(output_dir, "hardness_predictor"), exist_ok=True
                )
                torch.save(
                    self.hardness_predictor.state_dict(),
                    os.path.join(
                        output_dir, "hardness_predictor", "pytorch_model.bin"
                    ),
                )
    ...
    def _move_model_to_device(self, is_evaluating=False):
        ...
        if self.hans_enabled and self.hardness_predictor is not None:
            self.hardness_predictor.to(self.device)

    def _initialize_hans_predictor(self, model_name):
        hidden_size = (
            self.query_config.hidden_size
            if hasattr(self, "query_config") and self.query_config is not None
            else self.context_config.hidden_size
        )
        self.hardness_predictor = HardnessPredictor(hidden_size)
        predictor_path = None
        if model_name:
            predictor_path = os.path.join(
                model_name, "hardness_predictor", "pytorch_model.bin"
            )
        if predictor_path and os.path.isfile(predictor_path):
            state_dict = torch.load(predictor_path, map_location="cpu")
            self.hardness_predictor.load_state_dict(state_dict)
